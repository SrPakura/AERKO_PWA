// src/modules/progress_entry/views/record-measures.js
...
export class ProgressRecordMeasures extends HTMLElement {
    constructor() {
        super();
        this.sequence = [
            'neck', 'shoulder', 'chest', 'arm_relaxed', 
            'arm_flexed', 'waist', 'hip', 'thigh', 'calf'
        ];
    }
    ...
    async connectedCallback() {
        ...
        // 1. Cargamos el diccionario ANTES de que el componente haga nada
    ...
    _loadDraft() {
        ...
        const defaultOpts = { unit: 'CM', options: 'CM, IN' };
        
        this.data = {
            neck:        { value: draft.neck?.value || "",        ...defaultOpts, label: this.dict.t('record_meas_neck') },
            shoulder:    { value: draft.shoulder?.value || "",    ...defaultOpts, label: this.dict.t('record_meas_shoulder') },
            chest:       { value: draft.chest?.value || "",       ...defaultOpts, label: this.dict.t('record_meas_chest') },
            arm_relaxed: { value: draft.arm_relaxed?.value || "", ...defaultOpts, label: this.dict.t('record_meas_arm_rel') },
            arm_flexed:  { value: draft.arm_flexed?.value || "",  ...defaultOpts, label: this.dict.t('record_meas_arm_flex') },
            waist:       { value: draft.waist?.value || "",       ...defaultOpts, label: this.dict.t('record_meas_waist') },
            hip:         { value: draft.hip?.value || "",         ...defaultOpts, label: this.dict.t('record_meas_hip') },
            thigh:       { value: draft.thigh?.value || "",       ...defaultOpts, label: this.dict.t('record_meas_thigh') },
            calf:        { value: draft.calf?.value || "",        ...defaultOpts, label: this.dict.t('record_meas_calf') }
        };
    ...
    render() {
        this.innerHTML = `
            <style>
                ...
                /* ZONA DE BOTONES (EMPUJADA AL FONDO) */
...
customElements.define('progress-record-measures', ProgressRecordMeasures);